...
class CalcMesh:
    ...
    def __init__(self, nodes_coords, tetrs_points):
        ...
        self.j = 0
        self.smth = np.cos(self.nodes[0, :]) + self.j + np.sin(self.nodes[1, :])

        # Тут может быть скорость, но сейчас здесь нули
        self.velocity = np.zeros(shape=(3, int(len(nodes_coords) / 3)), dtype=np.double)
        w = 5
        x0 = self.nodes[0].mean()
        z0 = self.nodes[2].mean()

        self.velocity[1] = -w*(self.nodes[0] - x0*np.ones((1,514)))*(self.nodes[2] - x0*np.ones((1,514)))/10
        self.velocity[0] = w * (self.nodes[1] - x0 * np.ones((1, 514)))*(self.nodes[2] - x0*np.ones((1,514)))/10
    ...
    def move(self, tau):
        ...
        self.j += 0.1
        self.smth = np.cos(self.nodes[0, :]) + self.j
...
try:
    ...
    gmsh.merge(os.path.join(path, 'Custom_Test_Cube.stl'))
except:
